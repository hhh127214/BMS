...
#ifdef _WIN32
#include <process.h>
// [BMS 移植修补] 原 RT_DB 无条件 `typedef int pid_t;`，在 MSVC 下可以
// （MSVC 的 <process.h> 不定义 pid_t），但 MinGW 的 <process.h> 会经由
// <sys/types.h> 引入 `typedef _pid_t pid_t;` → conflicting types 编译失败。
// 因此仅在**非 MinGW** 的 Windows 工具链下补定义。
